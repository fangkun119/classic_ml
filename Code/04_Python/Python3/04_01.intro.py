...
# calculate residual : difference between label (y) and predicted value (t0 * x^2 + t1 * x + t2)
# residual = y - (t0 * x^2 + t1 * x + t2)
def residual(t, x, y):
    return y - (t[0] * x ** 2 + t[1] * x + t[2])

# calculate residual : difference between label (y) and predicted value (t0 * sin(t1 * x) + t2)
def residual2(t, x, y):
    print(t[0], t[1])
    return y - (t[0]*np.sin(t[1]*x) + t[2])

# return an array with element equal to 
#   x ** x         if x > 0
#   (-x) ** (-x)   if x < 0
def f(x):
    # init y: all elements are 1, but the shape is the same as x
    y = np.ones_like(x)
    # fill y: elememt is x ** x if x > 0; (-x) ** (-x) if x < 0
    i = x > 0
...
if __name__ == "__main__":
    # get a two-dimension numpy array like this
    # [[ 0  1  2  3  4  5]
    #  [10 11 12 13 14 15]
    #  [20 21 22 23 24 25]
    #  [30 31 32 33 34 35]
